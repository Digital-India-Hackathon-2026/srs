 ...
 * Priority: Gemini Vision → OpenAI Vision → Tesseract.js → graceful manual-entry fallback.
 *
 * Two modes:
...
async function isTesseractAvailable() {
  ...
  try {
    ...
    tesseractAvailable = typeof mod.recognize === "function";
...
async function callTesseract(buffer) {
  try {
    const Tesseract = await import("tesseract.js");
    const { data } = await Tesseract.recognize(buffer, "eng+tel+hin", {
      logger: isDev ? (m) => { if (m.status === "recognizing text") log(`Tesseract: ${Math.round(m.progress * 100)}%`); } : undefined,
    });
    return data.text || "";
  } catch (e) {
    log("Tesseract OCR failed:", e.message);
    return "";
  }
}
...
export async function extractStructuredFields(buffer, mimeType = "image/jpeg", docType = "aadhaar") {
  ...
  // Try Tesseract.js fallback (only if Vision AI unavailable or both failed)
  const tesseractOk = await isTesseractAvailable();
  if (tesseractOk) {
    try {
      log("Trying Tesseract.js fallback...");
      const raw = await callTesseract(buffer);
      if (raw && raw.length > 10) {
        log("Tesseract extraction successful", `${raw.length} chars`);
        return { fields: null, raw, method: "tesseract", error: null };
      }
      log("Tesseract returned too little text");
    } catch (e) {
      log("Tesseract.js failed:", e.message);
    }
  } else {
    log("Tesseract.js not available — skipping");
  }

  // Fallback — no OCR method available, allow manual entry
  const fallbackMsg = !geminiKey && !openaiKey && !tesseractOk
    ? "OCR is not available because no Vision API keys (GEMINI_API_KEY / OPENAI_API_KEY) or Tesseract.js are configured. Please enter the details manually."
    : "Automatic document reading is currently unavailable. Please enter the details manually.";
  log(fallbackMsg);
  return { fields: null, raw: "", method: "failed", error: fallbackMsg };
}
