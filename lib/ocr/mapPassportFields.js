...
export function mapPassportFields(aadhaarFields = {}, panFields = {}) {
  ...
  // ── State — default to Telangana for Hyderabad-area PINs ──
  if (aadhaarFields.pinCode && /^5\d{5}$/.test(aadhaarFields.pinCode)) {
    setField("state", "Telangana", "Aadhaar", 0.85);
  }

  // ── Try to detect district from address text ──
  if (aadhaarFields.address) {
    ...
    for (const d of districts) {
      if (addrLower.includes(d.toLowerCase())) {
        setField("district", d, "Aadhaar", 0.7);
        break;
      }
    }
  }
