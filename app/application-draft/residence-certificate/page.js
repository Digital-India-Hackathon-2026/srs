...
const UPLOAD_SLOTS = [
  { id: "aadhaar", label: "Aadhaar Card", hint: "Primary identity and address proof — Aadhaar address must match residence", required: true },
  { id: "residenceProof", label: "Address Proof (Utility Bill within 1 year)", hint: "Electricity bill / Water bill / Gas bill / Telephone bill (not older than 1 year)", required: true },
  { id: "rationCard", label: "Ration Card", hint: "White / Pink ration card showing current address", required: true },
...
const QUESTION_GROUPS = [
  { title: "Applicant Details", id: "applicant", fields: [
    ...
    { id: "caste", label: "Caste Category", type: "select", options: ["General (OC)","OBC","BC-A","BC-B","BC-C","BC-D","BC-E","SC","ST"], required: false },
    { id: "religion", label: "Religion", type: "text", required: false },
    { id: "nationality", label: "Nationality", type: "text", required: true },
  ]},
  { title: "Residence Address (current)", id: "address", fields: [
    { id: "houseNumber", label: "House / Door Number", type: "text", required: true },
    { id: "street", label: "Street / Colony / Locality", type: "text", required: true },
    ...
    { id: "yearsAtAddress", label: "How long residing at this address?", type: "select", options: ["Less than 1 year","1–3 years","3–5 years","5–7 years","More than 7 years"], required: true },
  ]},
  { title: "Supporting Details", id: "support", fields: [
    { id: "rationCardNumber", label: "Ration Card Number", type: "text", required: true },
    { id: "utilityAccountNumber", label: "Utility Bill Consumer / Account Number", type: "text", required: false },
    { id: "documentDate", label: "Date of Utility Bill / Address Proof", type: "date", required: false },
  ]},
  { title: "Purpose of Certificate", id: "purpose", fields: [
    { id: "purposeOfCertificate", label: "Purpose / Why is this certificate needed?", type: "select", options: ["Educational Admission","Government Job / Domicile Quota","Welfare Scheme","Property Registration","Bank / Financial Purpose","Court / Legal Purpose","Voter Registration","Passport Application","Other"], required: true },
    { id: "institutionName", label: "Institution / Organisation Name (if applicable)", type: "text", required: false },
  ]},
];

export default function ResidenceCertificateDraftPage() {
  ...
  function getFullDraft() { return { ...extractedFields, ...answers, state: answers.state || extractedFields.state || "Telangana", nationality: answers.nationality || extractedFields.nationality || "Indian" }; }

  function getReadiness() {
    const d = getFullDraft();
    const req = ["applicantName","dateOfBirth","gender","fatherName","aadhaarNumber","mobileNumber","houseNumber","street","address","city","district","state","pinCode","yearsAtAddress","rationCardNumber","purposeOfCertificate"];
  ...
  async function handleExtract() {
    ...
    try {
      ...
      const res = await fetch("/api/ocr/residence-certificate", { method: "POST", body: formData, signal: controller.signal });
  ...
  function copyFullDraft() {
    const d = getFullDraft();
    const lines = ["RESIDENCE CERTIFICATE APPLICATION DRAFT — SevaSetu Telangana","Submit via: https://meeseva.telangana.gov.in","═".repeat(55),"","── APPLICANT ──",`Name: ${d.applicantName||""}`,`DOB: ${d.dateOfBirth||""}`,`Gender: ${d.gender||""}`,`Father/Husband: ${d.fatherName||""}`,`Aadhaar: ${d.aadhaarNumber||""}`,`Mobile: ${d.mobileNumber||""}`,`Caste: ${d.caste||""}`,`Religion: ${d.religion||""}`,`Nationality: ${d.nationality||"Indian"}`,"","── RESIDENCE ADDRESS ──",`H.No/Door: ${d.houseNumber||""}`,`Street: ${d.street||""}`,`${d.address||""}`,`${d.city||""}, ${d.district||""}, ${d.state||""} - ${d.pinCode||""}`,`Years Residing: ${d.yearsAtAddress||""}`,"","── SUPPORTING DOCUMENTS ──",`Ration Card No: ${d.rationCardNumber||""}`,`Utility Acc No: ${d.utilityAccountNumber||""}`,`Document Date: ${d.documentDate||""}`,"","── PURPOSE ──",`Purpose: ${d.purposeOfCertificate||""}`,`Institution: ${d.institutionName||""}`,"","═".repeat(55),"Disclaimer: Draft for reference only. Submit at meeseva.telangana.gov.in","Generated by SevaSetu AI"];
  ...
  if (phase === PHASE_UPLOAD) return (
    <div className="min-h-screen flex flex-col"><Header />
      <div className="bg-[#1a3a5c] px-4 py-8 text-white text-center">
        <nav className="text-xs text-gray-400 mb-2 flex items-center justify-center gap-1"><Link href="/" className="hover:text-white">Home</Link><span>/</span><Link href="/application-draft" className="hover:text-white">Draft Generator</Link><span>/</span><span className="text-gray-200">Residence Certificate</span></nav>
        <h1 className="text-2xl font-black">Residence Certificate Application Draft</h1>
  ...
  const draftSections = [
    { title: "Applicant Details", fields: [{ label: "Full Name", id: "applicantName" },{ label: "Date of Birth", id: "dateOfBirth" },{ label: "Gender", id: "gender" },{ label: "Father/Husband Name", id: "fatherName" },{ label: "Aadhaar Number", id: "aadhaarNumber" },{ label: "Mobile", id: "mobileNumber" },{ label: "Email", id: "emailId" },{ label: "Caste Category", id: "caste" },{ label: "Religion", id: "religion" },{ label: "Nationality", id: "nationality" }] },
    { title: "Residence Address", fields: [{ label: "House / Door Number", id: "houseNumber" },{ label: "Street / Colony", id: "street" },{ label: "Full Address", id: "address" },{ label: "City / Mandal", id: "city" },{ label: "District", id: "district" },{ label: "State", id: "state" },{ label: "PIN Code", id: "pinCode" },{ label: "Years Residing", id: "yearsAtAddress" }] },
    { title: "Supporting Documents", fields: [{ label: "Ration Card Number", id: "rationCardNumber" },{ label: "Utility Account Number", id: "utilityAccountNumber" },{ label: "Document Date", id: "documentDate" }] },
