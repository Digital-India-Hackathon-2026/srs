...
import {
  ...
  ClipboardCopy, Download, ExternalLink, FileText, Info, Printer, Upload, User,
} from "lucide-react";
...
export default function VoterIdDraftPage() {
  ...
  const handleDocUpload = async (e) => {
    ...
    try {
      ...
      const res = await fetch("/api/ocr/voter-id", { method: "POST", body: fd });
      const data = await res.json();

      if (data.success && data.fields) {
        setFormData(prev => ({ ...prev, ...data.fields }));
        setUploadStatus("success");
        setToast(language === "te" ? "పత్రం నుండి డేటా విజయవంతంగా సేకరించబడింది!" : language === "hi" ? "दस्तावेज़ से डेटा सफलतापूर्वक निकाला गया!" : "Data extracted successfully from document!");
        setTimeout(() => setToast(""), 3000);
      } else {
        setUploadStatus("error");
        setToast(data.error || (language === "te" ? "పత్రం నుండి డేటా సేకరించలేకపోయాము" : language === "hi" ? "दस्तावेज़ से डेटा नहीं निकल सका" : "Could not extract data from document"));
        setTimeout(() => setToast(""), 4000);
      }
    } catch (err) {
      setUploadStatus("error");
      setToast(language === "te" ? "అప్‌లోడ్ విఫలమైంది" : language === "hi" ? "अपलोड विफल हुआ" : "Upload failed");
      setTimeout(() => setToast(""), 3000);
    } finally {
      setUploading(false);
    }
  ...
  function copyFullDraft() {
    ...
    const lines = [
      "VOTER ID APPLICATION DRAFT — SevaSetu Telangana",
      "Submit on: https://voters.eci.gov.in",
      "═".repeat(50),
      "",
      "── APPLICATION TYPE ──",
      `Form: ${formType === "form6" ? "Form 6 — New Voter Registration (Domestic)" : "Form 6A — Overseas Elector"}`,
      "",
      "── PERSONAL DETAILS ──",
      `First Name: ${formData.firstName || ""}`,
      `Last Name: ${formData.lastName || ""}`,
      `Father/Mother/Husband Name: ${formData.relativeName || ""}`,
      ...
      `${formData.town || ""}, ${formData.district || ""}, ${formData.state || ""} - ${formData.pinCode || ""}`,
      `Post Office: ${formData.postOffice || ""}`,
      "",
      "── CONSTITUENCY ──",
      `State: ${formData.constituencyState || ""}`,
      `District: ${formData.constituencyDistrict || ""}`,
      `Assembly Constituency: ${formData.constituencyName || ""}`,
    ];
    if (formType === "form6a") {
      lines.push(
        "",
        "── ADDRESS IN INDIA ──",
        `${formData.indiaHouseNo || ""}, ${formData.indiaStreet || ""}`,
        `${formData.indiaTown || ""}, ${formData.indiaDistrict || ""} - ${formData.indiaPinCode || ""}`,
        `Post Office: ${formData.indiaPostOffice || ""}`,
        "",
        "── PASSPORT DETAILS ──",
        `Passport No: ${formData.passportNo || ""}`,
        `Place of Issue: ${formData.passportPlace || ""}`,
        `Issue Date: ${formData.passportIssue || ""}`,
        `Expiry Date: ${formData.passportExpiry || ""}`,
        "",
        "── VISA DETAILS ──",
        `Visa No: ${formData.visaNo || ""}`,
        `Type: ${formData.visaType || ""}`,
        `Issue Date: ${formData.visaIssue || ""}`,
        `Expiry Date: ${formData.visaExpiry || ""}`,
        `Authority: ${formData.visaAuthority || ""}`,
        "",
        "── ADDRESS ABROAD ──",
        `${formData.abroadHouseNo || ""}, ${formData.abroadStreet || ""}`,
        `${formData.abroadTown || ""}, ${formData.abroadState || ""}, ${formData.abroadCountry || ""} - ${formData.abroadZip || ""}`,
        `Reason for Absence: ${formData.reasonAbsence || ""}`,
        `Date of leaving India: ${formData.absenceDate || ""}`,
      );
    }
    lines.push(
      "",
      "── DOCUMENT CHECKLIST ──",
      ...docs.map((doc, i) => `  ${docChecks[i] ? "✓" : "○"} ${doc.en || doc}`),
      "",
      "── DECLARATION ──",
    ...
    const draftSections = [
      {
        ...
        fields: [
          { label: "Form Type", value: formType === "form6" ? "Form 6 — New Voter Registration (Domestic)" : "Form 6A — Overseas Elector" },
        ],
      },
      {
        ...
        fields: [
          ...
          { id: "relativeName", label: "Father/Mother/Husband Name" },
      ...
      },
      {
        ...
        fields: [
          ...
          { id: "street", label: "Street / Area / Locality" },
      ...
      },
      {
        title: "Constituency",
        fields: [
          { id: "constituencyState", label: "State" },
          { id: "constituencyDistrict", label: "District" },
          { id: "constituencyName", label: "Assembly Constituency" },
        ],
      },
    ];
    if (formType === "form6a") {
      draftSections.push(
        {
          ...
          fields: [
            { id: "indiaHouseNo", label: "House No." },
            { id: "indiaStreet", label: "Street / Area / Locality" },
            { id: "indiaTown", label: "Town / Village" },
            { id: "indiaPostOffice", label: "Post Office" },
            { id: "indiaDistrict", label: "District" },
            { id: "indiaPinCode", label: "PIN Code" },
          ],
        },
        {
          ...
          fields: [
            ...
            { id: "passportPlace", label: "Place of Issue" },
            { id: "passportIssue", label: "Date of Issue" },
            { id: "passportExpiry", label: "Date of Expiry" },
          ],
        },
        {
          title: "Visa Details",
          fields: [
            { id: "visaNo", label: "Visa Number" },
            { id: "visaIssue", label: "Date of Issue" },
            { id: "visaExpiry", label: "Date of Expiry" },
            { id: "visaType", label: "Type of Visa" },
            { id: "visaAuthority", label: "Issuing Authority" },
          ],
        },
        {
          title: "Address Abroad",
          fields: [
            { id: "abroadHouseNo", label: "House No." },
            { id: "abroadStreet", label: "Street / Area" },
            { id: "abroadTown", label: "Town / City" },
            { id: "abroadState", label: "State / Province" },
            { id: "abroadCountry", label: "Country" },
            { id: "abroadZip", label: "Zip Code" },
            { id: "reasonAbsence", label: "Reason for Absence" },
            { id: "absenceDate", label: "Date of Leaving India" },
          ],
        },
      );
    }
      ...
      <div className="min-h-screen flex flex-col print:block">
        ...
        <div className="bg-white border-b border-gray-200 px-4 py-3 print:hidden sticky top-0 z-10">
          <div className="max-w-3xl mx-auto flex items-center justify-between flex-wrap gap-2">
            <span className="text-sm font-bold text-[#1a3a5c]">Voter ID Application Draft</span>
            <div className="flex gap-2">
              <button onClick={() => setShowPreview(false)} className="flex items-center gap-1 text-xs font-semibold text-gray-500 border border-gray-300 px-3 py-1.5 rounded-lg hover:bg-gray-50">
                <ChevronLeft size={11} /> {L.edit}
              </button>
        ...
        <main className="flex-1 max-w-3xl mx-auto w-full px-4 py-6 space-y-4">
          ...
          {draftSections.map(sec => (
            <div key={sec.title} className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
              ...
              <div className="p-4 space-y-1.5">
                {sec.fields.map((f, i) => {
                  const value = f.value !== undefined ? f.value : (draft[f.id] || "");
                  if (!value) return null;
                  return (
                    <div key={f.id || i} className="flex items-center justify-between py-1.5 border-b border-gray-50 last:border-0">
          ...
          <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
            <div className="bg-[#1a3a5c] text-white text-xs font-bold uppercase tracking-wide px-4 py-2">Document Checklist</div>
            <div className="p-4 space-y-2">
              {(formType === "form6" ? DOCS_FORM6 : DOCS_FORM6A).map((doc, i) => (
                <div key={i} className="flex items-start gap-2">
                  {docChecks[i] ? (
                    <CheckCircle2 size={14} className="text-green-500 mt-0.5 flex-shrink-0" />
                  ) : (
                    <span className="w-3.5 h-3.5 border border-gray-300 rounded mt-0.5 flex-shrink-0" />
                  )}
                  <span className="text-xs text-gray-700">{doc[language] || doc.en}</span>
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="p-4">
              {declared ? (
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-medium text-gray-800">Declaration confirmed</span>
                </div>
              ) : (
                <span className="text-xs text-red-400 italic">Declaration not confirmed</span>
              )}
            </div>
          </div>
          ...
          <div className="text-center pt-2 print:hidden">
            <a href="https://voters.eci.gov.in" target="_blank" rel="noopener noreferrer"
              ...
              <ExternalLink size={14} /> Open Official ECI Portal
            </a>
          </div>
        </main>
