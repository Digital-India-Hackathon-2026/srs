...
const UPLOAD_SLOTS = [
  { id: "hospitalRecord", label: "Hospital Death Record / Medical Certificate of Cause of Death", hint: "From hospital or attending physician", required: true },
  { id: "applicantAadhaar", label: "Applicant's Aadhaar Card (Informant)", hint: "Person applying for the death certificate", required: true },
  { id: "deceasedId", label: "Deceased's Identity Proof (Aadhaar / Voter ID / Passport)", hint: "For deceased person's identity verification", required: true },
  { id: "cremationCert", label: "Cremation / Burial Certificate (if available)", hint: "From cremation ground or burial ground authority", required: false },
];

const QUESTION_GROUPS = [
  { title: "Deceased Person Details", id: "deceased", fields: [
    { id: "deceasedName", label: "Name of Deceased", type: "text", required: true },
    { id: "deceasedGender", label: "Gender", type: "radio", options: ["Male", "Female", "Transgender"], required: true },
    { id: "deceasedDOB", label: "Date of Birth of Deceased (DD/MM/YYYY)", type: "text", required: false },
    { id: "ageAtDeath", label: "Age at Time of Death", type: "text", required: true },
    { id: "dateOfDeath", label: "Date of Death (DD/MM/YYYY)", type: "text", required: true },
    { id: "timeOfDeath", label: "Time of Death", type: "text", required: false },
    { id: "placeOfDeath", label: "Place of Death (Hospital / Residence / Other)", type: "text", required: true },
    { id: "causeOfDeath", label: "Cause of Death (if known)", type: "text", required: false },
    { id: "deceasedOccupation", label: "Occupation of Deceased", type: "text", required: false },
    { id: "deceasedNationality", label: "Nationality", type: "text", required: true },
    { id: "deceasedReligion", label: "Religion", type: "text", required: false },
  ]},
  { title: "Deceased's Address", id: "deceased-address", fields: [
    { id: "deceasedAddress", label: "Last Residence Address of Deceased", type: "text", required: true },
    { id: "deceasedCity", label: "City / Town / Village", type: "text", required: true },
    { id: "deceasedDistrict", label: "District", type: "text", required: true },
    { id: "deceasedState", label: "State", type: "text", required: true },
    { id: "deceasedPinCode", label: "PIN Code", type: "text", required: true },
  ]},
  { title: "Applicant / Informant Details", id: "applicant", fields: [
    { id: "applicantName", label: "Applicant (Informant) Name", type: "text", required: true },
    { id: "applicantRelation", label: "Relationship to Deceased", type: "select", options: ["Spouse","Son","Daughter","Father","Mother","Brother","Sister","Relative","Hospital Authority","Other"], required: true },
    { id: "applicantAddress", label: "Applicant's Address", type: "text", required: true },
  ...
  { title: "Registration Details", id: "registration", fields: [
    { id: "registrationPlace", label: "Place of Registration (Municipality / Gram Panchayat)", type: "text", required: false },
    { id: "reasonForApplying", label: "Purpose / Reason for Applying", type: "select", options: ["Legal / Property matters","Insurance claim","Pension / Bank closure","Family records","Other"], required: false },
  ]},
];

export default function DeathCertificateDraftPage() {
  ...
  function getFullDraft() { return { ...extractedFields, ...answers, deceasedNationality: answers.deceasedNationality || extractedFields.deceasedNationality || "Indian" }; }

  function getReadiness() {
    const draft = getFullDraft();
    const required = ["deceasedName","deceasedGender","ageAtDeath","dateOfDeath","placeOfDeath","deceasedAddress","deceasedPinCode","deceasedDistrict","deceasedState","applicantName","applicantRelation","applicantAddress","pinCode","mobileNumber"];
  ...
  async function handleExtract() {
    ...
    try {
      ...
      if (files.applicantAadhaar) formData.append("applicantAadhaar", files.applicantAadhaar);
      if (files.deceasedId) formData.append("deceasedId", files.deceasedId);
      const res = await fetch("/api/ocr/death-certificate", { method: "POST", body: formData, signal: controller.signal });
  ...
  function copyFullDraft() {
    ...
    const lines = ["DEATH CERTIFICATE APPLICATION DRAFT — SevaSetu Telangana","Submit on: https://crsorgi.gov.in","═".repeat(55),"","── DECEASED DETAILS ──",`Name: ${d.deceasedName||""}`,`Gender: ${d.deceasedGender||""}`,`Age at Death: ${d.ageAtDeath||""}`,`Date of Death: ${d.dateOfDeath||""}`,`Time of Death: ${d.timeOfDeath||""}`,`Place of Death: ${d.placeOfDeath||""}`,`Cause of Death: ${d.causeOfDeath||""}`,`Nationality: ${d.deceasedNationality||"Indian"}`,"","── DECEASED ADDRESS ──",`${d.deceasedAddress||""}`,`${d.deceasedCity||""}, ${d.deceasedDistrict||""}, ${d.deceasedState||""} - ${d.deceasedPinCode||""}`,"","── APPLICANT / INFORMANT ──",`Name: ${d.applicantName||""}`,`Relation: ${d.applicantRelation||""}`,`Address: ${d.applicantAddress||""}`,`Mobile: ${d.mobileNumber||""}`,`Email: ${d.emailId||""}`,"","═".repeat(55),"Disclaimer: Draft for reference only. Submit officially at crsorgi.gov.in or municipal office.","Generated by SevaSetu AI"];
  ...
  if (phase === PHASE_UPLOAD) return (
    <div className="min-h-screen flex flex-col"><Header />
      <div className="bg-[#1a3a5c] px-4 py-8 text-white text-center">
        <nav className="text-xs text-gray-400 mb-2 flex items-center justify-center gap-1"><Link href="/" className="hover:text-white">Home</Link><span>/</span><Link href="/application-draft" className="hover:text-white">Draft Generator</Link><span>/</span><span className="text-gray-200">Death Certificate</span></nav>
        <h1 className="text-2xl font-black">Death Certificate Application Draft</h1>
      ...
      <main className="flex-1 max-w-2xl mx-auto w-full px-4 py-8 space-y-5">
        ...
        <button onClick={handleExtract} disabled={processing || (!files.hospitalRecord && !files.applicantAadhaar && !files.deceasedId)} className="w-full bg-[#1a3a5c] hover:bg-[#0f2540] disabled:opacity-40 text-white font-bold py-3.5 rounded-xl text-sm flex items-center justify-center gap-2"><Sparkles size={16} /> Extract Details & Generate Draft</button>
  ...
  const draftSections = [
    { title: "Deceased Person", fields: [{ label: "Name", id: "deceasedName" },{ label: "Gender", id: "deceasedGender" },{ label: "DOB", id: "deceasedDOB" },{ label: "Age at Death", id: "ageAtDeath" },{ label: "Date of Death", id: "dateOfDeath" },{ label: "Time of Death", id: "timeOfDeath" },{ label: "Place of Death", id: "placeOfDeath" },{ label: "Cause of Death", id: "causeOfDeath" },{ label: "Occupation", id: "deceasedOccupation" },{ label: "Religion", id: "deceasedReligion" },{ label: "Nationality", id: "deceasedNationality" }] },
    { title: "Deceased's Address", fields: [{ label: "Address", id: "deceasedAddress" },{ label: "City/Town", id: "deceasedCity" },{ label: "District", id: "deceasedDistrict" },{ label: "State", id: "deceasedState" },{ label: "PIN Code", id: "deceasedPinCode" }] },
    { title: "Applicant / Informant", fields: [{ label: "Name", id: "applicantName" },{ label: "Relationship", id: "applicantRelation" },{ label: "Address", id: "applicantAddress" },{ label: "District", id: "district" },{ label: "State", id: "state" },{ label: "PIN Code", id: "pinCode" },{ label: "Mobile", id: "mobileNumber" },{ label: "Email", id: "emailId" }] },
    { title: "Registration", fields: [{ label: "Place of Registration", id: "registrationPlace" },{ label: "Reason for Applying", id: "reasonForApplying" }] },
  ];

  return (
    <div className="min-h-screen flex flex-col print:block"><Header />
      <div className="bg-white border-b border-gray-200 px-4 py-3 print:hidden sticky top-0 z-10"><div className="max-w-3xl mx-auto flex items-center justify-between flex-wrap gap-2"><div><span className="text-sm font-bold text-[#1a3a5c]">Death Certificate Draft</span><span className={`ml-2 text-[10px] font-bold px-2 py-0.5 rounded ${readiness.pct >= 90 ? "bg-green-100 text-green-700" : readiness.pct >= 60 ? "bg-yellow-100 text-yellow-700" : "bg-red-100 text-red-700"}`}>{readiness.status} ({readiness.pct}%)</span></div>
