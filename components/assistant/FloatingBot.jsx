...
export default function FloatingBot() {
  ...
  return (
    <>
      ...
      {isOpen && (
        <div
          className="fixed z-50 shadow-2xl rounded-xl overflow-hidden flex flex-col border border-gray-200/80 no-print chatbot-window"
          style={{
            bottom: "108px",
            right: "16px",
      ...
      {/* Floating area */}
      <div className="fixed bottom-4 right-3 sm:bottom-6 sm:right-6 z-50 flex flex-col items-end gap-2 no-print">
        {showBubble && !isOpen && (
          <div className="speech-bubble bg-white text-[#163A63] text-xs font-medium px-3 py-2 rounded-xl shadow-lg border border-gray-100 relative max-w-[160px] sm:max-w-[180px] leading-relaxed cursor-pointer" onClick={handleOpen}>
            {speechBubble}
        ...
        {minimized && !isOpen && (
          <button onClick={handleOpen} className="bg-white text-[#163A63] text-xs font-semibold px-3 py-1.5 rounded-full shadow-lg border border-gray-200 flex items-center gap-1.5 hover:shadow-xl transition-shadow mb-1 min-h-[44px]">
        ...
        <button
          ...
          onMouseEnter={() => { if (!isOpen) { setHovered(true); setMascotState("hover"); } }}
          onMouseLeave={() => { setHovered(false); if (!isOpen) setMascotState("idle"); }}
        ...
        >
          {isOpen ? (
            ...
            <SevasetuMascot state={mascotState} size={hovered ? 82 : 68} />
          )}
        </button>
      </div>
